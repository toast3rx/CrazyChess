...
/**
 * Get number of moves that pawns can make
 * First element in the pair represents number of diagonal attacks + number of pushes
 * Second element in the pair represents number of  pushes that are promotions
 * Note: The behavior is undefined for attacks that are promotions
 * For promotions, it is added only a single move, not one for each promotion
 * @param side
 * @param blackPieces
 * @param whitePieces
 * @param allPieces
 * @param enPassantWhite
 * @param enPassantBlack
 * @return
 */
std::pair<int, int> Pawns::getNumberOfMoves(PlaySide side, uint64_t allPieces) {

    // first - number of attacks
    // second - number of promotions
    std::pair<int, int> moves = {0, 0};

    moves.first = Utils::count_set_bits(getAllAttacks(side));
...
void Pawns::getAttacks(std::vector<Move *> &moves, PlaySide side, uint64_t blackPieces, uint64_t whitePieces,
                       uint64_t enPassantWhite, uint64_t enPassantBlack) {
